# Cuestión 8.19: Modifica el código para que los parámetros se pasen a través de
# los registros $a0 - $a1 y el de salida a través de $v0. Supón que el programa que hace
# la llamada pretende que después de ejecutar la subrutina estos registros contengan el
# mismo contenido que el que se ha cargado inicialmente en el programa.
...
lw   $a0, 4($fp)    # se lee en s0, lo que hay una posicion abajo del fp,la "direcc del vect"
lw   $a1, 8($fp)    # se lee en s1,dos pos abajo de donde apunta fp, la variable "n1"
and  $v0, $v0, $0

#bucle de cuenta de elementos nulos
bucle: 
beq  $a1, $0, finb   # si s1 = 0 saltar a finb
lw   $s2, 0($a0)     # cargar s2 = Mem(s0)
bne  $s2, $0, finsi  # si s2<>0 saltar a finsi
addi $v0, $v0, 1     # v0=s2
 
finsi: 
addi $a0, $a0, 4     # s0 = s0 + 4 ( se apunta al sig elem del vector )
addi $a1, $a1, -1    # s1 = s1 - 1 (contador del bucle, al inicio vale : 4)
